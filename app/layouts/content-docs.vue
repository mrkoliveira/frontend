			...
			<TairoTopnavHeader>
				<TairoTopnavContent class="[--tairo-topnav-content-width:1536px] flex items-center">
					<div class="px-4 lg:hidden">
						<BaseButton size="icon-sm" variant="ghost" rounded="md" @click="isMobileOpen = !isMobileOpen">
					...
					<div class="shrink-0 lg:me-12 flex items-center gap-x-2">
						<NuxtLink to="/" class="block">
							<AppLogoText class="h-6 w-auto hidden md:block text-primary-500 px-3" />
							<AppLogo class="size-7 block md:hidden w-auto text-primary-500" />
						</NuxtLink>
					</div>
					...
					<div class="ms-auto flex items-center justify-end gap-x-3">
						...
						>
							<div class="pointer-events-none">
								<span class="font-sans text-sm"> Search docs... </span>
							</div>
							<div class="flex gap-1">
								<BaseKbd size="sm" variant="default" class="!font-semibold h-6!"> Ctrl </BaseKbd>
								<BaseKbd size="sm" variant="default" class="!px-2 !font-semibold h-6!"> K </BaseKbd>
							</div>
						</div>
						<BaseButton size="icon-sm" variant="ghost" rounded="md" class="md:hidden" @click="searchOpen = !searchOpen">
							<Icon name="lucide:search" class="size-4 text-muted-700 dark:text-muted-300" />
						</BaseButton>
					...
					<ul class="fixed top-14 w-60 max-h-[calc(100dvh_-_3.5rem)] pt-10 pb-16 pe-4 overflow-y-auto nui-slimscroll">
						<li v-for="item in navigation" :key="item.path">
							<div v-if="item.children">
								...
								<div>
									<ul>
										<li v-for="link in item.children" :key="link.path" class="relative mt-1">
											<div v-if="link.children" class="mt-6">
												<BaseHeading size="xs" weight="semibold" class="uppercase tracking-wide mt-2.5 ms-2.5 mb-2 text-muted-600 dark:text-muted-400">
													<span>{{ link.title }}</span>
												</BaseHeading>
												<ul>
													<li v-for="sublink in link.children" :key="sublink.path" class="relative mt-1">
														<NuxtLink
															:to="sublink.path"
