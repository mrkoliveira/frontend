	...
	<TairoSidenavLayout v-slot="{ toggleMobileNav }">
		<TairoSidenavSidebar>
			<TairoSidenavSidebarHeader>
				<NuxtLink to="/">
					<AppLogoText class="text-primary-500 h-6 w-auto" />
				</NuxtLink>
			...
			<TairoSidenavSidebarLinks class="p-4 grow">
				<template v-for="item in menu" :key="item.label">
					<TairoSidenavSidebarLink v-if="!item.children" :to="item.to" :icon="item.icon" :label="item.label" />
					<TairoSidenavCollapsible v-else :default-open="item.children.some((child) => child.to === $route.path) || undefined">
						<template #trigger>
							<TairoSidenavCollapsibleTrigger :icon="item.icon" :label="item.label" />
						</template>
						<TairoSidenavCollapsibleLink v-for="child in item.children" :key="child.label" :to="child.to" :label="child.label" />
				...
				<TairoSidenavSidebarDivider />
				<BaseHeading size="xs" weight="medium" class="uppercase mt-2 mb-3"> Recently viewed </BaseHeading>
				...
				<TairoSidenavSidebarDivider />
				<BaseHeading size="xs" weight="medium" class="uppercase mt-2 mb-3"> Companies </BaseHeading>
