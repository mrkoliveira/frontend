...
<template>
	<div class="dark:bg-muted-900 flex min-h-screen w-full flex-col bg-white sm:flex-row dark:[--color-input-default-bg:var(--color-muted-950)]">
		...
		<aside class="absolute z-10 flex h-16 items-center ps-3 lg:hidden">
			<button class="text-muted-900 disabled:text-muted-400" type="button" @click="isMobileOpen = !isMobileOpen">
		...
		<aside
			...
			:class="[isCollapsed ? 'w-(--sidebar-width-collapsed)' : 'w-(--sidebar-width)', isMobileOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0']"
		>
			<div class="border-muted-200 dark:border-muted-800 fixed inset-y-0 start-0 z-auto flex h-svh flex-col border-r transition-all duration-300" :class="isCollapsed ? 'lg:w-(--sidebar-width-collapsed)' : 'w-(--sidebar-width)'">
				<div class="flex h-svh flex-1 translate-x-[-0.5px] flex-col transition-all duration-300" :class="isCollapsed ? 'w-(--sidebar-width-collapsed)' : 'w-(--sidebar-width)'">
					<div class="relative flex items-center p-2 pb-1">
						<div class="flex items-center gap-2.5">
							<NuxtLink to="/">
								<AppLogo class="text-primary-500 size-8" />
							</NuxtLink>
						</div>
						<div class="flex-1" />
						<div class="absolute z-10 transition-all duration-300 group-hover/sidebar:opacity-100" :class="isCollapsed ? 'opacity-0 -start-6 hidden' : 'w-(--sidebar-width) start-[calc(var(--sidebar-width)-(calc(var(--spacing)*10)))] opacity-100'">
							<BaseButton size="icon-sm" rounded="md" @click="isCollapsed = !isCollapsed">
					...
					<div class="transition-sidebar grid w-full min-w-0 p-2">
						<div class="transition-sidebar flex w-full min-w-0 flex-col gap-2">
							<BaseButton rounded="md" size="sm" class="w-full font-medium" variant="dark" @click="clearMessages">
								<Icon v-if="isCollapsed" name="lucide:plus" class="size-4 shrink-0" />
							...
							<div class="group relative flex items-center rounded-md">
								...
								<Icon name="lucide:plus" class="absolute end-2 top-1/2 size-4 shrink-0 -translate-y-1/2 opacity-0 transition-opacity duration-300" :class="isCollapsed ? 'hidden' : 'group-hover:opacity-100'" />
							</div>
							<div class="group relative flex items-center rounded-md">
								<NuxtLink
									class="group-hover:bg-muted-100 dark:group-hover:bg-muted-800 text-muted-600 dark:text-muted-500 hover:text-muted-900 focus:text-muted-900 dark:hover:text-muted-100 dark:focus:text-muted-100 focus-visible:text-muted-900 dark:focus-visible:text-muted-100 peer relative flex h-8 w-full items-center justify-start! gap-2 overflow-hidden rounded-lg px-2 pe-0! text-start text-sm font-normal transition-all duration-300 focus-visible:pe-10 group-hover:pe-10"
					...
					<div class="flex flex-1 flex-col overflow-auto">
						<div class="grid w-full min-w-0 p-2 transition-all duration-300" :class="isCollapsed ? 'hidden' : ''">
							<div class="text-muted-400 dark:text-muted-300 border-muted-200 dark:border-muted-800 flex h-9 items-center border-t px-2 pt-4 text-[0.8125rem] font-medium transition-all duration-300" :class="isCollapsed ? '-mt-9' : ''">Recent Chats</div>
							<ul class="mt-3 space-y-1">
								<li v-for="(chat, i) in chats" :key="i" class="group/item relative">
									<button
										type="button"
										class="text-muted-600 group-hover/item:text-muted-900 dark:text-muted-500 dark:group-hover/item:text-muted-200 group-hover/item:bg-muted-100 dark:group-hover/item:bg-muted-800 flex w-full items-center gap-2 rounded-md px-3 py-1.5 text-start font-sans text-sm transition-colors duration-300"
									>
									...
									<div class="absolute end-0 top-0">
										...
										>
											...
											<BaseDropdownItem to="#" title="Share" text="Share this chat" rounded="sm">
												<template #start>
													<Icon name="lucide:share" class="me-2 block size-4" />
												</template>
											</BaseDropdownItem>
											<BaseDropdownItem to="#" title="Rename" text="Rename this chat" rounded="sm">
												<template #start>
													<Icon name="lucide:edit-3" class="me-2 block size-4" />
												</template>
											</BaseDropdownItem>
											<BaseDropdownItem to="#" title="Archive" text="Archive this chat" rounded="sm">
												<template #start>
													<Icon name="lucide:archive" class="me-2 block size-4" />
												</template>
											</BaseDropdownItem>
											<BaseDropdownItem to="#" title="Delete" text="Delete this chat" rounded="sm">
												<template #start>
						...
						<div class="transition-sidebar mt-auto grid w-full min-w-0 p-2">
							<BaseButton size="icon-sm" rounded="md" :class="isCollapsed ? 'flex' : 'hidden'" @click="isCollapsed = !isCollapsed">
								<Icon name="solar:siderbar-linear" class="size-4" />
							</BaseButton>
						</div>
					</div>
						...
						<div class="group relative flex items-center rounded-md transition-all duration-300" :class="isCollapsed ? 'w-8' : 'w-full'">
							...
							>
								<div class="absolute inset-2 flex w-fit items-center justify-center gap-2 whitespace-nowrap" :class="isCollapsed ? 'start-2' : ''">
									<BaseAvatar size="xxs" src="/img/avatars/2.svg" class="relative" :class="isCollapsed ? '-start-1' : ''" />
		...
		<main class="relative z-auto min-h-screen grow">
			<div class="pointer-events-none absolute inset-0 start-1/2 z-auto flex size-full max-w-screen-2xl -translate-x-1/2 items-center justify-center" :class="conversation.length > 0 ? 'opacity-0' : ''">
				<AppLogoText class="h-16 -translate-y-40 p-2 text-black/5 transition-opacity duration-200 md:-translate-y-36 dark:text-white/5" />
			</div>
			<div class="absolute inset-0 z-auto flex items-center justify-center overflow-hidden sm:p-4">
				<div class="absolute end-5 top-5.5">
					<BaseTag size="sm" rounded="full" class="scale-90 font-medium flex gap-1">
				...
				<!-- Content -->
				<div class="absolute z-20 flex min-h-[285px] w-full flex-col items-stretch justify-start sm:min-h-[270px]" :class="conversation.length === 0 ? 'top-1/2 -translate-y-1/2' : 'top-0 h-screen'">
					<div v-if="conversation.length > 0" class="dark:bg-muted-900 flex h-16 items-center justify-between bg-white px-6">
						<div class="flex items-center gap-3">
							<button type="button" class="font-sans text-sm font-medium underline-offset-4 hover:underline" @click="clearMessages">New Chat</button>
							<BaseTag size="sm" rounded="full" class="scale-90 font-medium flex gap-1 items-center justify-center">
						...
						<div class="flex items-center gap-3">
							...
							<BaseDropdown rounded="lg" label="Dropdown" orientation="start" class="group/dropdown">
								<template #button>
					...
					<div v-if="conversation.length === 0" class="mx-auto mb-6 flex max-w-[49rem] flex-col items-center gap-2 px-6">
						<BaseHeading as="h1" size="lg" weight="semibold" class="text-muted-900 text-pretty text-center tracking-tighter sm:text-xl! md:text-3xl! dark:text-white"> What do you want to ship? </BaseHeading>
						<h2 class="text-muted-700 dark:text-muted-500 text-balance text-center text-sm">Generate UI, ask questions, debug, execute code, and much more.</h2>
					</div>
					<!-- Conversation -->
					<div v-else ref="chatEl" class="nui-slimscroll relative mx-auto size-full max-w-[49rem] grow overflow-y-auto px-6">
						<div class="relative min-h-full w-full transition-opacity duration-150 ease-in h-[600px]">
													...
													<div class="flex items-center">
														<div class="relative size-6 items-center justify-center overflow-visible">
															<div class="absolute inset-0 flex items-center justify-center">
																<AppLogo class="text-muted-400 size-6" />
															</div>
														</div>
													</div>
													<div class="min-w-0 flex-1 translate-y-[0.5px]">
														<div class="flex min-w-0 flex-1 flex-col items-stretch gap-6">
															<div class="flex-1 text-sm">
																<div>
																	...
																	<div v-else class="prose prose-sm prose-muted dark:prose-invert min-w-0 break-words" v-html="item.response" />
																</div>
															</div>
															<div v-if="!loading" class="flex items-center gap-2">
																<button
																	class="border-muted-100 dark:border-muted-950 dark:bg-muted-950 bg-muted-100 text-muted-900 dark:text-muted-100 hover:bg-muted-200 focus-visible:bg-muted-200 focus:bg-muted-200 dark:hover:bg-muted-800 dark:focus-visible:bg-muted-800 dark:focus:bg-muted-800 inline-flex h-7 shrink-0 cursor-pointer items-center justify-center gap-1 whitespace-nowrap text-nowrap rounded-lg border px-2 text-xs font-medium outline-none"
																>
																	<Icon name="solar:copy-linear" class="size-4" />
					...
					<div class="flex items-center">
						<div class="mx-auto flex w-full max-w-[49rem] flex-col px-6">
							<div v-if="upgrade" class="bg-muted-100 dark:bg-muted-800 text-muted-600 rounded-t-xl">
								<div class="text-muted-900 flex items-center justify-between gap-2 py-1.5 ps-3 pe-2 text-xs md:text-sm">
									...
									<div class="flex items-center gap-1">
										<button
											type="button"
											class="text-primary-500 inline-flex h-5 shrink-0 cursor-pointer items-center justify-center gap-1.5 whitespace-nowrap text-nowrap rounded-lg bg-transparent p-0! px-3 font-sans text-sm font-medium underline-offset-2 outline-none transition-all hover:underline"
										>
											Upgrade Plan
										</button>
							...
							<form @submit.prevent="addMessage">
								<BaseTextarea v-model="input" v-focus rounded="md" placeholder="Ask Tairo a question…" autogrow :class="[upgrade && 'rounded-t-none!']" />
								...
								<div class="flex items-center justify-between mt-4">
									...
									<div class="flex items-center gap-2">
										<BaseButton size="icon-sm" rounded="md" variant="dark" :loading="loading" type="submit">
					...
					<div v-if="conversation.length === 0" class="mx-auto flex h-20 min-h-0 max-w-[49rem] shrink-0 items-center justify-center px-6 sm:h-16">
						<div class="flex flex-wrap justify-center gap-3">
							<h2 class="sr-only">Suggested Chat Messages</h2>
							<button
							...
							>
								...
								<svg height="16" stroke-linejoin="round" viewBox="0 0 16 16" width="16" class="text-current">
					...
					<div v-else>
						<p class="text-muted-500 py-2 text-center text-xs">TairoGPT may make mistakes. Please use with discretion.</p>
					</div>
				</div>
				<footer v-if="conversation.length === 0" class="absolute bottom-4 text-xs">
					<nav class="divide-muted-300 dark:divide-muted-700 mx-auto flex h-4 max-w-[49rem] divide-x px-6">
						<NuxtLink class="text-muted-600 hover:text-muted-900 dark:text-muted-500 dark:hover:text-muted-100 flex items-center gap-1 px-2 transition-colors" to="/"> FAQ </NuxtLink>
						<NuxtLink class="text-muted-600 hover:text-muted-900 dark:text-muted-500 dark:hover:text-muted-100 flex items-center gap-1 px-2 transition-colors" to="/"> Tairo </NuxtLink>
						<NuxtLink class="text-muted-600 hover:text-muted-900 dark:text-muted-500 dark:hover:text-muted-100 flex items-center gap-1 px-2 transition-colors" to="/"> Terms </NuxtLink>
						<NuxtLink class="text-muted-600 hover:text-muted-900 dark:text-muted-500 dark:hover:text-muted-100 flex items-center gap-1 px-2 transition-colors" to="/"> AI Policy </NuxtLink>
						<NuxtLink class="text-muted-600 hover:text-muted-900 dark:text-muted-500 dark:hover:text-muted-100 flex items-center gap-1 px-2 transition-colors" to="/"> Privacy </NuxtLink>
					</nav>
				</footer>
			</div>
		</main>
		...
		<!-- Overlay -->
		<div v-if="isMobileOpen" role="button" tabindex="0" class="bg-muted-950/80 fixed start-0 top-0 z-[29] size-full starting:opacity-0 transition-opacity duration-300 lg:pointer-events-none! lg:opacity-0!" @click="isMobileOpen = false" />
	</div>
</template>
