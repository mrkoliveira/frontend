<script setup lang="ts">
	...
	if (import.meta.dev && !config.public.mapboxToken) {
		console.warn('NUXT_PUBLIC_MAPBOX_TOKEN environment variable is not defined, mapbox features are disabled')
	}
	...
	function getMapStyle() {
		switch (mode.value) {
			case 'base':
				return colorMode.value === 'dark' ? 'mapbox://styles/mapbox/dark-v11' : 'mapbox://styles/mapbox/light-v11'
			case 'navigation':
				return colorMode.value === 'dark' ? 'mapbox://styles/mapbox/navigation-night-v1' : 'mapbox://styles/mapbox/navigation-day-v1'
			case 'satellite':
		...
		if (!placeLayer) {
			map.value.addLayer({
				...
				paint: {
					...
					// ],
					'circle-color': '#536ddd',
					'circle-radius': ['interpolate', ['linear'], ['zoom'], 11, ['case', ['==', ['feature-state', 'selected'], true], 5, ['==', ['feature-state', 'highlight'], true], 7, 6], 16, ['case', ['==', ['feature-state', 'selected'], true], 8, ['==', ['feature-state', 'highlight'], true], 10, 9]],
					...
					'circle-stroke-opacity': 0.2,
					'circle-stroke-width': ['interpolate', ['linear'], ['zoom'], 11, ['case', ['==', ['feature-state', 'selected'], true], 5, ['==', ['feature-state', 'highlight'], true], 3, 4], 16, ['case', ['==', ['feature-state', 'selected'], true], 7, ['==', ['feature-state', 'highlight'], true], 5, 6]]
				}
			})
		}
...
<template>
	<div class="map-container flex -mt-6 h-[calc(100dvh_-_56px)] flex-col lg:flex-row">
		<!-- Sidebar -->
		<div class="dark:bg-muted-900 w-full shrink-0 bg-muted-50 h-[calc(100dvh_-_56px)] lg:w-96">
			<div class="nui-slimscroll overflow-y-auto p-6 h-[calc(100dvh_-_56px)]!">
				<!-- Title -->
				<BaseHeading size="xs" weight="medium" class="mb-4 uppercase tracking-wider flex">
				...
				<div class="flex flex-col gap-4">
					...
					<template v-else-if="places?.features">
						<BaseCard
							...
							:class="[selectedLocation?.id === feature.properties?.id && 'border-primary-500']"
							tabindex="0"
							role="button"
						...
						>
							<div class="relative">
								<div class="relative mb-3 flex items-center gap-2">
									...
									<div class="font-sans">
										<h4 class="text-muted-800 dark:text-muted-100 text-sm font-medium leading-none">
											{{ feature.properties?.name }}
										</h4>
										<p class="text-muted-400 text-xs">Open until {{ feature.properties?.openingCount }}</p>
									</div>
								...
								<div v-if="selectedLocation?.id === feature.properties?.id" class="mt-3 flex items-center gap-2">
									<BaseButton size="sm" variant="primary"> Schedule a trip </BaseButton>
									<BaseButton size="sm" variant="muted"> Details </BaseButton>
									<div class="ms-auto text-muted-400 flex items-center gap-1 font-sans text-xs">
										<Icon name="lucide:flag" class="size-3" />
										<span class="dark-inverted"> {{ feature.properties?.distance }} miles </span>
									</div>
								</div>
							</div>
						</BaseCard>
		...
		<div class="relative grow h-[calc(100dvh_-_56px)]">
			<div v-if="selectedLocation" ref="popupElement" class="starting:opacity-0 transition-discrete duration-300 transition-all">
				<DemoDemoMapMarker v-bind="(selectedLocation.properties as unknown as GeoJsonLocationProperties)" @close="selectFeature()" />
			</div>
			<div ref="mapElement" class="absolute inset-0 size-full">
				<div class="absolute inset-0 bg-muted-100 dark:bg-muted-800 flex items-center justify-center flex-col">
					...
					<template v-else-if="hasError">
						<DevOnly>
							<BaseHeading size="lg"> Invalid Mapbox token. </BaseHeading>
							<BaseParagraph class="text-muted-500"> Set `NUXT_PUBLIC_MAPBOX_TOKEN` in the .env file. </BaseParagraph>
						</DevOnly>
					</template>
				</div>
			</div>
