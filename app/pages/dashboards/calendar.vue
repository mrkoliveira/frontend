<script setup lang="ts">
	...
	import { categoryTheme, datesToHeight, dateToTop, topToDate, useCalendarEvents, useCreateEvent, useDateRange, useDragEvent, useDragEventPending, useNowMarker, useViewPan } from '~/utils/bundles/calendar'
	import { addMinutes, endOfDay, format, isPast, isToday } from '~/utils/bundles/date-fns'
	...
	const { isHeightDragging, isPositionDragging, onHeightDragStart, onPositionDragStart } = useDragEvent(
		settings,
		calendarEvents,
		// on drag end
		async (event) => {
	...
	const selectedEvent = computed(() => {
		return calendarEvents.value.find((event) => event?.customData?.id === selectedEventId.value)?.customData || pendingEvents.value.find((event) => event.id === selectedEventId.value)
	})
	...
	const { isPendingEventDragging, onPendingEventDragStart } = useDragEventPending(
		...
		(event) => {
			...
			const idx = pendingEvents.value.findIndex((item) => item.id === event.customData.id)
			if (idx === -1) {
				return
			}
	...
	const isDragging = computed(() => isPendingEventDragging.value || isPositionDragging.value || isHeightDragging.value)
...
<template>
	<div class="flex w-full select-none h-[calc(100dvh_-_85px)]!">
		<div class="border-muted-200 dark:border-muted-800 flex w-full border-t -mt-6">
			<!-- scrollable area -->
			<div ref="scrollCalendarRef" class="nui-slimscroll relative w-full flex flex-col grow space-y-14 overflow-y-auto lg:w-auto lg:overflow-x-hidden">
				<!-- sticky header -->
				<div role="button" tabindex="-1" class="dark:bg-muted-900 flex bg-muted-50 mb-0!" @click="() => scrollCalendarToTop()">
					<div class="border-muted-200 dark:border-muted-800 w-10 border-b" />
					<div class="border-muted-200 dark:border-muted-800 grid grow border-b" :class="[settings.hideWeekends ? 'grid-cols-5' : 'grid-cols-7']">
						<span v-for="day of weekdays" :key="day.toISOString()" class="day-label text-muted-900 dark:text-muted-400 pointer-events-none flex h-[52px] items-center gap-2 p-3 text-sm" :class="[isPast(endOfDay(day)) ? 'opacity-50' : '']">
				...
				<div class="flex h-[calc(100dvh_-_85px)]! overflow-y-auto nui-slimscroll">
					...
					<div class="w-10">
						<div class="h-16" />
						<div>
							<div class="border-muted-200 dark:border-muted-800 pointer-events-none grow border-t">
								<div
								...
								>
									<span class="dark:bg-muted-900 text-muted-900 dark:text-muted-300 absolute -top-2 start-1 bg-muted-50 px-2 text-xs"> {{ i + (settings.hourOpen - 1) }}h </span>
								</div>
							</div>
						</div>
					</div>
              ...
              "
						...
						>
							<template #header-start-button="{ movePrev }">
								<BaseButton rounded="full" size="icon-sm" @click="movePrev" @keydown.space.enter="movePrev">
									<Icon name="lucide:chevron-left" class="top-auto! h-4! w-4!" />
								</BaseButton>
							</template>
							<template #header-right-button="{ moveNext }">
								<BaseButton rounded="full" size="icon-sm" @click="moveNext" @keydown.space.enter="moveNext">
								...
								>
									...
									<template v-for="event in attributes" :key="event.key">
										<div
											...
											:style="{
												top: `${dateToTop(settings, event.customData.startDate, day.date)}px`,
												height: `${datesToHeight(settings, event.customData.startDate, event.customData.endDate, day.date) + 4}px`
											}"
										/>
										...
										<div
											...
											:style="{
												top: `${dateToTop(settings, event.customData.startDate, day.date)}px`,
												height: `${datesToHeight(settings, event.customData.startDate, event.customData.endDate, day.date)}px`
											}"
											@click.stop="() => onSelectEvent(event.customData)"
										>
											<DemoDemoCalendarEvent :event="event" @positiondrag="(e) => onPositionDragStart(event, e)" />
										</div>
										<div
											...
											:style="{
												top: `${dateToTop(settings, event.customData.startDate, day.date) + datesToHeight(settings, event.customData.startDate, event.customData.endDate, day.date)}px`
											}"
											:class="{
												'pointer-events-none': isDragging
											}"
			...
			<!-- Right panel -->
			<div class="bg-white dark:bg-muted-900 border-muted-200 dark:border-muted-800 hidden flex flex-col w-68 h-[calc(100dvh_-_60px)]! shrink-0 border-s sm:block overflow-hidden">
				<!-- toolbar -->
				...
				<!-- settings -->
				<div v-if="showSettings" class="dark:bg-muted-900 border-muted-300 dark:border-muted-800 border-y bg-white p-4 shrink-0">
					<div class="grid grid-cols-2 gap-x-2 gap-y-4">
						<BaseField v-slot="{ inputAttrs, inputRef }" label="Open from">
							<BaseInputNumber :ref="inputRef" v-bind="inputAttrs" v-model="settings.hourOpen" :min="0" :max="settings.hourClose - 1" size="sm" />
						</BaseField>
						<BaseField v-slot="{ inputAttrs, inputRef }" label="Close at">
							<BaseInputNumber :ref="inputRef" v-bind="inputAttrs" v-model="settings.hourClose" :min="settings.hourOpen + 1" :max="24" size="sm" />
						</BaseField>
						<BaseField v-slot="{ inputAttrs, inputRef }" label="Precision" class="col-span-2">
							<BaseInputNumber :ref="inputRef" v-bind="inputAttrs" v-model="settings.hourPrecision" :min="5" :step="5" :max="30" size="sm" />
						</BaseField>
					</div>
					<BaseField v-slot="{ inputAttrs, inputRef }" label="Week starts on" class="mt-4">
						<div class="grid grid-cols-3 items-end gap-x-2 gap-y-4">
							<BaseSelect :ref="inputRef" v-bind="inputAttrs" v-model="settings.weekStartsOn" size="sm" class="col-span-2">
								<BaseSelectItem :value="0"> Sunday </BaseSelectItem>
								<BaseSelectItem :value="1"> Monday </BaseSelectItem>
								<BaseSelectItem :value="2"> Tuesday </BaseSelectItem>
								<BaseSelectItem :value="3"> Wednesday </BaseSelectItem>
								<BaseSelectItem :value="4"> Thursday </BaseSelectItem>
								<BaseSelectItem :value="5"> Friday </BaseSelectItem>
								<BaseSelectItem :value="6"> Saturday </BaseSelectItem>
							</BaseSelect>
							<div class="flex items-center *:first:border-e-0 *:first:rounded-e-none *:last:rounded-s-none">
								<BaseButton rounded="lg" size="icon-sm" :variant="settings.hourHeight === 480 ? 'muted' : 'default'" @click="() => updateHeight(480)">
									<Icon name="carbon:maximize" class="size-4" />
								</BaseButton>
								<BaseButton rounded="lg" size="icon-sm" :variant="settings.hourHeight === 160 ? 'muted' : 'default'" @click="() => updateHeight()">
					...
					<div class="mt-4">
						<BaseSwitchThin v-model="settings.hideWeekends" label="Hide weekends" />
					</div>
				</div>
				...
				>
					...
					>
						<div v-if="showSettings" role="button" tabindex="0" class="absolute inset-0 z-50 cursor-pointer backdrop-blur-[2px]" @click="showSettings = false" />
						<div class="flex items-center justify-between mb-4">
							<BaseHeading size="xs" weight="medium" lead="snug" class="text-muted-600 dark:text-muted-400 uppercase uppercase">
								<span>Event Info</span>
							</BaseHeading>
							<BaseButton size="icon-sm" class="dark:bg-muted-800 bg-white scale-90" @click="selectedEventId = undefined">
								<Icon name="lucide:x" class="size-4" />
							</BaseButton>
						</div>

						<div class="flex flex-col gap-2">
							<BaseField v-slot="{ inputAttrs, inputRef }" label="Title">
								<BaseInput :ref="inputRef" v-bind="inputAttrs" v-model.trim="selectedEvent.title" v-focus />
							</BaseField>

							<BaseField v-slot="{ inputAttrs, inputRef }" label="Category">
								<BaseSelect
									:ref="inputRef"
									v-bind="inputAttrs"
							...
							<BaseField label="Assignee">
								...
								<template #default="{ inputAttrs, inputRef }">
									<BaseAutocomplete :ref="inputRef" v-bind="inputAttrs" v-model="selectedEvent.participants" multiple icon="ph:users-three" placeholder="Search..." clearable>
										<BaseAutocompleteItem v-for="item in users" :key="item.id" :value="item" :text-value="item.name">
											<span class="flex gap-2">
												<BaseAvatar :src="item.photo" :alt="item.name" size="xxs" />
					...
					>
						<div v-if="showSettings" tabindex="0" role="button" class="absolute inset-0 z-50 cursor-pointer backdrop-blur-[2px]" @click="showSettings = false" />
              ...
              "
						>
							<template v-if="pendingEvents?.length">
								<BaseHeading size="xs" weight="medium" lead="snug" class="text-muted-600 dark:text-muted-400 mb-4 uppercase"> Pending events </BaseHeading>
								<Draggable v-for="pendingEvent in pendingEvents" :key="pendingEvent.id">
									<DemoDemoCalendarEventPending :event="pendingEvent" role="button" @click="() => onSelectEvent(pendingEvent as any)" />
								</Draggable>
							</template>
							<div v-else>
								<!-- empty state -->
								<div class="p-4">
									<img class="mx-auto block max-w-[200px] dark:hidden" src="/img/illustrations/placeholders/flat/placeholder-projects.svg" alt="" />
									<img class="mx-auto hidden max-w-[200px] dark:block" src="/img/illustrations/placeholders/flat/placeholder-projects-dark.svg" alt="" />
									<div class="mt-4 text-center">
										<BaseHeading as="h4" size="lg" weight="medium" class="mb-1 text-muted-900 dark:text-white">
											<span>No pending events</span>
										</BaseHeading>
										<BaseParagraph size="xs" lead="tight" class="text-muted-600 dark:text-muted-400 mx-auto max-w-[200px] font-sans!">
											<span>You've planned all your events for this week.</span>
										</BaseParagraph>
										<NuxtLink href="#" class="text-primary-500 focus-visible:nui-focus dark:text-sunny pointer-events-auto mx-auto mt-2 flex items-center justify-center gap-1 font-sans text-xs underline-offset-4 hover:underline">
