		...
		<div class="mx-auto flex w-full max-w-sm flex-col gap-3 px-4">
			...
			<BaseCard rounded="lg" class="relative z-10 p-6">
				<div class="mb-4 text-center">
					<h3 class="tracking-wide text-muted-400 font-sans text-[0.65rem] font-semibold uppercase">Customer</h3>
				</div>
				<div>
					<div class="relative">
						<TairoInput v-if="!itemSelected" v-model="search" icon="lucide:search" rounded="lg" placeholder="ex: Figma, Github, ..." />
						<div v-else class="flex items-center gap-3">
							<BaseAvatar size="sm" :src="project.customer?.logo" class="bg-muted-100 dark:bg-muted-700/60" />
							<div class="flex flex-col">
								<h3 class="text-muted-800 dark:text-muted-100 font-sans text-sm font-semibold">
							...
							<div class="me-3 ms-auto">
								<BaseButton size="icon-sm" rounded="full" @click="dismissCustomer">
						...
						>
							<!-- Results -->
							<div v-if="filteredCustomers.length > 0" class="nui-slimscroll max-h-[248px] space-y-2 overflow-y-auto">
								<!-- Result -->
								<div v-for="customer in filteredCustomers" :key="customer.name" role="button" class="hover:bg-muted-100 flex cursor-pointer items-center gap-3 rounded-lg px-3 py-2">
									...
									<div class="ms-auto">
										<BaseButton size="icon-sm" rounded="full" @click="selectCustomer(customer)">
			...
			<BaseCard rounded="lg" class="p-6">
				<div class="mb-4 text-center">
					<h3 class="tracking-wide text-muted-400 font-sans text-[0.65rem] font-semibold uppercase">Time Frame</h3>
				</div>
				<div class="divide-muted-200 dark:divide-muted-700 flex w-full justify-between divide-x">
					<div class="relative pe-4">
						<LazyAddonDatepicker v-model="project.startDate" :masks="masks" :minute-increment="15" :min-date="new Date()" trim-weeks>
					...
					<div class="relative ps-4">
						<LazyAddonDatepicker id="endDate" v-model="project.endDate" :masks="masks" :minute-increment="15" :min-date="project.startDate" trim-weeks>
							<template #default="{ inputValue, inputEvents }">
								<div class="relative">
									<input
										class="text-muted-600 dark:text-muted-100 placeholder:text-muted-300 dark:placeholder:text-muted-600 focus-visible:outline-muted-300/70 peer inline-flex h-10 w-full items-center rounded-lg bg-transparent ps-10 font-sans text-sm leading-tight focus-visible:outline-dashed focus-visible:outline-offset-4"
			...
			<BaseCard rounded="lg" class="p-6">
				<div class="mb-4 text-center">
					<h3 class="tracking-wide text-muted-400 font-sans text-[0.65rem] font-semibold uppercase">Initial budget</h3>
				</div>
				<RadioGroupRoot v-model="budget" class="flex w-full justify-center gap-3">
					<RadioGroupItem value="< 5K" class="group">
						<BaseButton rounded="lg" class="group-data-[state=checked]:bg-primary-500! group-data-[state=checked]:border-primary-500! group-data-[state=checked]:shadow-primary-500/20 group-data-[state=checked]:text-white! group-data-[state=checked]:shadow-xl">
							<span>&lt;&nbsp;5K</span>
						</BaseButton>
					</RadioGroupItem>
					<RadioGroupItem value="< 30K" class="group">
						<BaseButton rounded="lg" class="group-data-[state=checked]:bg-primary-500! group-data-[state=checked]:border-primary-500! group-data-[state=checked]:shadow-primary-500/20 group-data-[state=checked]:text-white! group-data-[state=checked]:shadow-xl">
							<span>&lt;&nbsp;30K</span>
						</BaseButton>
					</RadioGroupItem>
					<RadioGroupItem value="< 100K" class="group">
						<BaseButton rounded="lg" class="group-data-[state=checked]:bg-primary-500! group-data-[state=checked]:border-primary-500! group-data-[state=checked]:shadow-primary-500/20 group-data-[state=checked]:text-white! group-data-[state=checked]:shadow-xl">
							<span>&lt;&nbsp;100K</span>
						</BaseButton>
					</RadioGroupItem>
