		...
		>
			<div class="flex w-1/2 items-center gap-2 md:w-1/5">
				<NuxtLink to="/" class="ms-2 inline-flex" aria-label="Go to Tairo homepage">
					<AppLogoText class="text-primary-500 group-[&.scrolled]/landing:h-6 group-[&:not(.scrolled)]/landing:h-7 motion-safe:transition-all motion-safe:duration-200" />
				</NuxtLink>
			</div>
			<div class="hidden dark:bg-muted-950 fixed inset-x-0 top-20 mx-auto w-[calc(100%_-_2rem)] items-center justify-center bg-white lg:static lg:mx-0 lg:flex lg:w-auto lg:flex-row lg:bg-transparent!">
					...
					<TairoMenuList class="flex-col lg:flex-row">
						<TairoMenuItem>
							<TairoMenuLink as-child>
								<NuxtLink to="/demos" active-class="text-primary-500"> Prebuilt pages </NuxtLink>
							</TairoMenuLink>
						</TairoMenuItem>
						<TairoMenuItem>
							<TairoMenuLink as-child>
								<NuxtLink to="/documentation" active-class="text-primary-500"> Documentation </NuxtLink>
							</TairoMenuLink>
						</TairoMenuItem>
					</TairoMenuList>
				</TairoMenu>
				<div class="px-6">
					...
					>
						<Icon name="lucide:search" class="size-4 motion-safe:transition-colors motion-safe:duration-300" />
						<span
			...
			<div class="flex w-1/2 items-center justify-end gap-4 md:w-1/5">
				<button
					...
					"
				>
					<Icon name="ph:drop-half-bottom-duotone" class="text-muted-400 size-5" />
				</button>
				<BaseThemeToggle aria-label="Toggle darkmode" />
				<BaseButton rounded="lg" variant="primary" to="https://go.cssninja.io/buy-tairo" class="hidden! lg:flex!"> Buy Tairo </BaseButton>
				<button type="button" class="flex items-center justify-center lg:hidden" :aria-label="isMobileOpen ? 'Close menu' : 'Open menu'" @click="isMobileOpen = !isMobileOpen">
					<div class="space-y-1.5">
						<span class="bg-primary-600 block h-0.5 motion-safe:transition-all motion-safe:duration-300" :class="isMobileOpen ? 'w-2' : 'w-6'" />
						<span class="bg-primary-600 block h-0.5 motion-safe:transition-all motion-safe:duration-300" :class="isMobileOpen ? 'w-6' : 'w-6'" />
						<span class="bg-primary-600 block h-0.5 motion-safe:transition-all motion-safe:duration-300" :class="isMobileOpen ? 'w-4' : 'w-6'" />
					</div>
				</button>
			</div>
		</div>
