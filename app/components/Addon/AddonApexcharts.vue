...
<script setup lang="ts">
	const props = defineProps<AddonAddonApexchartsProps>()
...
<template>
	<div ref="target">
		<div v-if="!isLoaded && !targetIsVisible" class="m-4 w-[calc(100%-32px)] flex items-center justify-center" :style="{ height: `${Number(height) - 32}px` }">
		...
		<ClientOnly>
			...
			<div v-else class="m-4 w-[calc(100%-32px)] text-center" :style="{ height: `${Number(height) - 32}px` }">
