...
<script setup lang="ts">
	const props = withDefaults(defineProps<AddonAddonMapboxLocationPickerProps>(), {
		rounded: 'md',
		geocoder: true,
		geolocation: true,
		marker: true,
	...
	const emits = defineEmits<AddonAddonMapboxLocationPickerEmits>()
	...
	watch([() => hash(props.options), () => hash(props.geocoder), () => hash(props.marker), () => hash(props.geolocation)], initMapbox)
	...
	async function initMapbox() {
		try {
			...
			if (!mapboxgl || !Geocoder) {
				...
				const [_MapboxGl, _MapboxGeocoder] = await Promise.all([import('mapbox-gl').then((m) => m.default), import('@mapbox/mapbox-gl-geocoder').then((m) => m.default)])
...
<template>
	<div class="location-picker relative overflow-hidden" :class="[props.rounded === 'none' && 'rounded-none', props.rounded === 'sm' && 'rounded-sm', props.rounded === 'md' && 'rounded-md', props.rounded === 'lg' && 'rounded-lg', props.rounded === 'xl' && 'rounded-xl']">
		...
		<div class="absolute end-2.5 top-2.5">
			<BaseButton v-if="props.geolocation" size="icon-md" :rounded="props.rounded === 'xl' ? 'lg' : props.rounded" class="shadow-xl shadow-muted-300/30 dark:shadow-muted-900/40" @click="getUsersLocation">
