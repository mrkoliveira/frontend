<script setup lang="ts">
	const props = withDefaults(
		defineProps<{
			...
			color?: 'success' | 'info' | 'warning' | 'danger' | 'primary' | 'lime' | 'sky'
		}>(),
		{
			rounded: 'sm',
			color: 'success'
...
<template>
	<div>
		<DemoDemoTabbedContent title="Tasks" :labels="['All', 'Mine']" :rounded="props.rounded">
			<template #tab-1>
				<BaseCheckboxGroup v-model="tasks" class="mb-2 space-y-6">
					<label v-for="task in todos" :key="task.id" class="text-muted-300 flex cursor-pointer items-center gap-3">
						<TairoCheckboxAnimated :value="`Option ${task.id}`" variant="success" />
						<div>
