<script setup lang="ts">
	...
	const { steps, currentStep, progress, goToStep } = useMultiStepForm<Project, ProjectStepData>()
	...
	<div class="dark:bg-muted-950 absolute start-0 top-0 h-16 w-full bg-white">
		<div class="relative flex h-16 w-full items-center justify-between px-4">
			<div class="flex items-center">
				<NuxtLink to="/dashboards" class="border-muted-200 dark:border-muted-800 flex w-14 items-center justify-center border-r pe-6">
					<AppLogo class="text-primary-600 h-8 shrink-0" />
				</NuxtLink>
				<div v-if="currentStep" class="hidden items-center gap-2 ps-6 font-sans sm:flex">
					<p class="text-muted-500 dark:text-muted-400">Step {{ currentStep.id + 1 }}:</p>
					<h2 class="text-muted-800 font-semibold dark:text-white">
						{{ currentStep.meta.name }}
					</h2>
				</div>
				<div ref="target" class="relative hidden sm:block">
					<button type="button" class="flex size-10 items-center justify-center" @click="openDropdown">
						<Icon name="lucide:chevron-down" class="text-muted-400 size-4 transition-transform duration-300" :class="open ? 'rotate-180' : ''" />
					</button>
					<div
						...
						:class="open ? 'opacity-100 translate-y-0' : 'opacity-0 pointer-events-none translate-y-1'"
					>
						<div class="space-y-1">
							<button
								v-for="step in steps"
								...
								"
							>
								<p class="text-muted-500 dark:text-muted-400 text-xs">Step {{ step.id + 1 }}:</p>
