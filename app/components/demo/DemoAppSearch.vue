<script setup lang="ts">
	...
	const hasResult = computed(() => Boolean(contentResults.value?.length || routesResults.value?.length))
			...
			>
				...
				<ComboboxRoot open ignore-filter :reset-search-term-on-blur="false" :reset-search-term-on-select="false">
					<BaseField class="px-2 pb-2">
						...
						<template #hint>
							<BaseText size="xs" class="opacity-60 ms-auto"> navigate with <BaseKbd size="sm" variant="muted"> ↑ </BaseKbd> and <BaseKbd size="sm" variant="muted"> ↓ </BaseKbd> </BaseText>
						</template>
					...
					<ComboboxContent v-if="hasResult || search" class="p-2 max-h-[50vh] nui-slimscroll overflow-y-auto space-y-6 py-4" @escape-key-down="isOpen = false">
						<ComboboxGroup v-if="isShurikenSearch">
							<ComboboxLabel class="px-2 mb-2">
								<BaseTag variant="muted"> Looking for Shuriken UI components? </BaseTag>
							</ComboboxLabel>
							<ComboboxItem value="https://shurikenui.com/docs/components" class="scroll-mt-2" as-child>
								<a href="https://shurikenui.com/docs/components" target="_blank">
									<DemoDemoAppSearchResult title="Shuriken UI" icon="nui-icon:shurikenui-icon" subtitle="View all Base components documentation" />
								</a>
							</ComboboxItem>
						</ComboboxGroup>
						<ComboboxEmpty v-else-if="search" class="text-center text-muted-400 p-4">
							<BaseParagraph size="sm" class="text-muted-400">
								Ops, no results found for "<BaseText weight="medium" size="sm">
									{{ search }} </BaseText
								>"
							</BaseParagraph>
							...
							<BaseParagraph size="xs" class="text-muted-400 dark:text-muted-500">
								...
								<BaseLink class="cursor-pointer" @click="search = 'form'">
									<BaseText weight="medium" class="text-muted-600 dark:text-muted-400" size="xs"> form </BaseText>
								</BaseLink>
								or
								<BaseLink class="cursor-pointer" @click="search = 'analytics'">
									<BaseText weight="medium" class="text-muted-600 dark:text-muted-400" size="xs"> analytics </BaseText>
								</BaseLink>
							</BaseParagraph>
						</ComboboxEmpty>
						...
						<ComboboxGroup v-if="contentResults?.length">
							<ComboboxLabel class="px-2 mb-2">
								<BaseTag variant="muted"> Documentation Hub </BaseTag>
							</ComboboxLabel>

							<ComboboxItem v-for="page in contentResults" :key="page?.id" :value="page?.id" class="scroll-mt-2" @select="handleSelect">
								<DemoDemoAppSearchResult :search="search" :title="page?.title" :prefix="page?.titles?.join(' > ') ?? ''" :subtitle="page?.content" />
							</ComboboxItem>
						</ComboboxGroup>

						<ComboboxGroup v-if="routesResults?.length">
							<ComboboxLabel class="px-2 mb-2">
								<BaseTag variant="muted"> Demo Pages </BaseTag>
							</ComboboxLabel>
							<ComboboxItem v-for="page in routesResults" :key="page?.name" :value="page?.id" class="scroll-mt-2" @select="handleSelect">
								<DemoDemoAppSearchResult :search="search" :title="page?.title" :subtitle="page?.description" />
							</ComboboxItem>
						</ComboboxGroup>
					</ComboboxContent>
				</ComboboxRoot>
