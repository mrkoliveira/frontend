...
<template>
	<BaseCard rounded="md" class="p-4 md:p-6">
		<div class="items-center justify-between flex mt-2">
			<BaseHeading as="h4" size="xs" weight="medium" lead="none" class="text-muted-700 dark:text-muted-100 uppercase"> Recent Transactions </BaseHeading>
			<DemoDemoLinkArrow to="#" label="View all" class="me-4" />
		</div>
		...
		<div v-else class="mt-7 overflow-x-auto pb-2">
			<table class="w-full whitespace-nowrap">
				...
				<tbody>
					<!-- Row -->
					<tr v-for="item in data?.data.slice(0, 8)" :key="item.id" tabindex="0" class="hover:bg-muted-50 dark:hover:bg-muted-700/50 transition-colors duration-100">
						...
						<td class="px-4 py-2">
							<BaseTag rounded="full" :variant="statusColor(item.status)" size="sm">
								{{ item.status }}
							</BaseTag>
						</td>
						<td class="ps-4 pe-2 py-2 rounded-e-md">
							<div class="text-muted-400 flex items-center justify-end gap-2">
								<BaseTag size="sm" weight="medium" lead="none" class="text-muted-400">
									{{ item.method }}
								</BaseTag>
							</div>
						</td>
