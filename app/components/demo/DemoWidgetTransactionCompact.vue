...
<template>
	<BaseCard rounded="md" class="py-4 px-2 md:py-6 md:px-4">
		...
		<div v-else class="mt-7 overflow-x-auto">
			<table class="w-full whitespace-nowrap">
				...
				<tbody>
					<!-- Row -->
					<tr v-for="item in data?.data" :key="item.id" tabindex="0" class="hover:bg-muted-200/50 dark:hover:bg-muted-900/50 transition-colors duration-100">
						...
						<td class="ps-4 pe-2 py-2 rounded-e-md text-end">
							<BaseTag :variant="statusColor(item.status)" rounded="full" size="sm">
								{{ item.status }}
							</BaseTag>
						</td>
					</tr>
